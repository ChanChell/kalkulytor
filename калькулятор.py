...
def text_to_number(text):
    ...
    while i < len(parts):
        ...
        if part in num_words:  # Проверяем, является ли слово числом
            if i + 1 < len(parts) and parts[i + 1] in num_words:
                ...
                i += 1
        elif part in ["плюсминус", "плюс", "минусминус", "минус", "умножитьминус", "умножить", "разделитьминус", "разделить"]:  # Проверяем, является ли слово операцией
    ...
    for i, operation in enumerate(operations):
        if operation == "умножитьминус":
            numbers[i] *= -numbers[i + 1]  # Выполняем умножение на отрицательное число
            ...
            numbers.pop(i + 1)  
            operations.pop(i)  
        elif operation == "разделитьминус":
            if numbers[i + 1] == 0:
                raise ValueError("На ноль делить нельзя!")  # Выбрасываем исключение, если деление на ноль
            numbers[i] /= -numbers[i + 1]  # Выполняем деление на отрицательное число
            numbers.pop(i + 1)  
            operations.pop(i)
